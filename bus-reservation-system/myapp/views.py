...
def findbus(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            context["error"] = "Некоректно заповненні поля"
            return render(request, 'myapp/findbus.html', context)
        if bus_list:
            return render(request, 'myapp/list.html', locals())
        else:
            context["error"] = "Рейсів не знайдено"
            return render(request, 'myapp/findbus.html', context)
    else:
    ...
    if request.method == 'POST':
        ...
        if bus:
            if bus.rem > int(seats_r):
                ...
                context["error"] = "Виберіть меншу кількість білетів"
                return render(request, 'myapp/findbus.html', context)
...
def cancellings(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            Book.objects.filter(id=id_r).update(status='Скасовано')
            Book.objects.filter(id=id_r).update(nos=0)
            return redirect(seebookings)
        except Exception as ex:
            context["error"] = "Ви не забронювали квитки на цей рейс"
...
def seebookings(request,new={}):
    ...
    if book_list:
        ...
        context["error"] = "На жаль, незнайдено заброньваних квитків"
        return render(request, 'myapp/findbus.html', context)
...
def signin(request):
    ...
    if request.method == 'POST':
        ...
        context["error"] = "Ви не авторизовані"
        return render(request, 'myapp/signin.html', context)
...
def signout(request):
    context = {}
    logout(request)
    context['error'] = "Ви вийшли з системи"
    return render(request, 'myapp/signin.html', context)
